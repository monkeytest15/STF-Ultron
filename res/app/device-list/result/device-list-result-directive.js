...
module.exports = function DeviceListResultDirective(
  $filter
, $compile
, $rootScope
, $http
, gettext
, DeviceResultcolumnService
, GroupService
, DeviceService
...
, StandaloneService
  ...
  return {
    restrict: 'E'
  , template: require('./device-list-result.jade')
  , scope: {
      tracker: '&tracker'
    , columns: '&columns'
    , sort: '=sort'
  ...
  , link: function(scope, element) {
      ...
      scope.columnDefinitions = DeviceResultcolumnService
      ...
      function createRow(device) {
        ...
        var result
        $http.get('/test/result/').success(function(data){
          result = data
          for (var i in data) {
            ...
            for(var j in data[i]){
              ...
              if(j=='starttime'){
                ...
                resultTd.innerHTML = '<a class=\'large button blue\' onclick=singleReuslt_click(this,\''+data[i][j]+'\')>'+Y+M+D+h+m+s+'</a>'
              }
              else{
                resultTd.innerHTML = data[i][j];
              }
        
              // console.log("circle j "+j)
              // console.log("test result:"+data[i][j])
            }
            console.log("tr----"+tr)
            reTr[i]=tr
          }
          for(var h in reTr){
            insertRow(reTr[h], device)
            // console.log("reTr----"+reTr[h])
          }
        })
