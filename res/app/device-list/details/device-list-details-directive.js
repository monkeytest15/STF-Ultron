...
, StandaloneService
  ...
  , link: function(scope, element) {
      ...
      function createRow(device) {
        ...
        // var newTd
        var runTd= tr.insertCell();
        if (device.usable){
          runTd.innerHTML = '<a class="large button blue" id="runBt" type="button" num="'+device.serial+'"onclick="run_click(this,\''+device.serial+'\');">run</a>';
        }
        else{
          if(device.state === 'busy'){
            runTd.innerHTML = '<a class="large button yellow" id="runBt" type="button" num="'+device.serial+'">busy</a>';
          }
          else{
            runTd.innerHTML = '<a class="large button red" id="runBt" type="button" num="'+device.serial+'">disconnect</a>';
          }
        }
        // var resultTd= tr.insertCell();
        // resultTd.innerHTML = '<input id="resultBt" type="button" value="result" num="'+device.serial+'"onclick="result_click(this,\''+device.serial+'\');">';
        
        mapping[id] = device

        return tr
      }
      function createIOSRow() {
        var total = rows.length
        $http.get('/device/ios').success(function(data){
          // console.log("yuanq:"+data);
          for(var re in data){
            var name = data[re].split('-')[0]
            var id = data[re].split('-')[1]
            var tr = document.createElement('tr')
            ...
            tr.id = id
            td0.innerHTML = '<strong>'+name+'</strong>';
            td1.innerHTML = '<strong>'+id+'</strong>';
            td2.innerHTML = '<a class="large button blue" id="runBt" type="button" num="'+id+'"onclick="run_click(this,\''+id+'\');">run</a>';
            // return tr
            tbody.insertBefore(tr, 0 < total ? rows[0] : null)
          }
