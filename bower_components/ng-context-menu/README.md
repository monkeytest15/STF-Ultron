...
#### Step 4 (Alternative): Add dynamic context menu
```html
<div context-menu context-menu-items="itemsArray" class="panel panel-default position-fixed" id="id"
     ng-class="{ 'highlight': highlight, 'expanded' : expanded }">
  ...
</div>
```
id field is mandatory!<br>
Your itemsArray contains objects of type:
```javascript
{
     title: 'item menu title',//the menu item name
     callback: function(){}//the callback to invoke when clicking this item
}
```
